...
public final class CookieUtil {
	 ...
	 * @return boolean
	 */
	public static boolean addCookie(HttpServletResponse response, String domain, String path, int maxAge, String name,
	 ...
	 * @return boolean
	 */
	public static boolean deleteCookie(HttpServletRequest request, HttpServletResponse response, String domain,
			String path, String name) {
		/*
		 * 쿠키 관리는 웹 클라이언트가 하기 때문에 쿠키를 삭제하는 명령어는 없다. 쿠키를 삭제하기 위해서는 쿠기의 maxAge(상태 유지시간 -
		 * 초단위 설정)를 0으로 만들어서 전송하면 된다.
		 */
		if (!StringUtil.isEmpty(name)) {
			Cookie cookie = getCookie(request, name);
	 ...
	 * @return boolean
	 */
	public static boolean deleteCookie(HttpServletRequest request, HttpServletResponse response, String path,
			String name) {
